export async function spotifyGetArt(albumId: string) {
  ...
  const url =
    process.env.NEXT_PUBLIC_API_URL + `external/albums?${params.toString()}`;
...
export async function spotifySearch(term: string, type: string) {
  ...
  const url =
    process.env.NEXT_PUBLIC_API_URL + `external/search?${params.toString()}`;
  ...
  try {
    ...
    if (userId && term.toLowerCase() == spotArtistName.toLowerCase()) {
      return userId;
    }
  } catch (error) {
    console.error(error);
