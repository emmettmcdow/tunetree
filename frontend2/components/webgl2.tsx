...
const Player = forwardRef(
  (_: unknown, ref: React.ForwardedRef<Object3D<Object3DEventMap>>) => {
    return (
...
const Cover: React.FC<{ image: string; camPos: Vector3 }> = ({
  image,
  camPos,
}) => {
...
function SkyBox() {
  const scene: Scene = useThree((state: RootState) => {
    return state.scene;
  });
...
// Scene components
const VinylScene: React.FC<{ image: string; dimensions: Array<number> }> = ({
  image,
  dimensions,
}) => {
...
const Vinyl: React.FC<{ image: string; camPos: Vector3 }> = ({
  image,
  camPos,
}) => {
  ...
  useFrame((state: RootState) => {
    if (
      typeof vinylref.current != "undefined" &&
      typeof camref != "undefined"
    ) {
      camref.position.x =
        camPos.x + circleSize * Math.cos(state.clock.elapsedTime);
      camref.position.y =
        camPos.y + circleSize * Math.sin(state.clock.elapsedTime);
...
const CubeScene: React.FC<{ image: string; dimensions: Array<number> }> = ({
  image,
  dimensions,
}) => {
  return (
