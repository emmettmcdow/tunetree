...
function IconLink({ n, m, className, provider, link, setLink }: { n?: number, m?: number, className?: string, provider: string, link: string, setLink: React.Dispatch<React.SetStateAction<string>>}) {
...
function CircleButtonBox({trackInfo, setLink}: {trackInfo: Track, setLink: React.Dispatch<React.SetStateAction<string>>}) {
        ...
        <div className={`relative`} style={displayStyle}>
          <div className="absolute left-0 top-1/12 flex flex-col items-left justify-normal z-30">
            <Tooltip text={tooltip}>
              <div className={"bg-black/30 flex items-center backdrop-blur-md py-2 px-4 my-2 rounded-lg z-30 w-48"}>
                <Image alt="album-art" src={track.image || "/placeholder.png" } className="w-16 m-2" height="1024" width="1024"/>
                <div className="flex-1">
                  <Marquee text={track.name + " / "} className="w-20"></Marquee>
                  <Marquee text={track.artist + " / "} className="w-20"></Marquee>
                </div>
              </div>
            </Tooltip>
            {buttons}
