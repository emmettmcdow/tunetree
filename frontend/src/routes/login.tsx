...
export function Password({name, password, setPassword}: {name: string, password: string, setPassword: Function}) {
  ...
  return (
    <div className="flex mb-2">
      ...
      <input className="w-full rounded-lg p-1" type={type} name={name} placeholder={name} value={password} onChange={(e) => setPassword(e.target.value)}/>
    ...
    <div className="h-screen flex flex-col">
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-indigo-200 w-3/4 p-4 rounded-lg">
        ...
        <form className="flex flex-col mb-2">
          ...
          <button className="w-full bg-emerald-500 rounded-lg"><span className="p-4 py-2 text-white text-xl">login</span></button>
        </form>
